#include "tq/new_parser.hpp"
#include <stdexcept>

namespace tq {

NewParser::NewParser(std::vector<Token> tokens) : tokens_(std::move(tokens)), pos_(0) {}

const Token& NewParser::current() const {
    return pos_ < tokens_.size() ? tokens_[pos_] : tokens_.back();
}

const Token& NewParser::peek(size_t offset) const {
    size_t peek_pos = pos_ + offset;
    return peek_pos < tokens_.size() ? tokens_[peek_pos] : tokens_.back();
}

void NewParser::advance() {
    if (!is_at_end()) ++pos_;
}

bool NewParser::is_at_end() const {
    return current().type == TokenType::Eof;
}

bool NewParser::check(TokenType type) const {
    return current().type == type;
}

bool NewParser::match(TokenType type) {
    if (check(type)) {
        advance();
        return true;
    }
...
Token NewParser::consume(TokenType type, const std::string& message) {
    if (check(type)) {
        Token tok = current();
        advance();
        return tok;
...
ExprPtr NewParser::parse_expression() {
    return parse_pipe();
}

// Pipe: expr | expr
ExprPtr NewParser::parse_pipe() {
...
ExprPtr NewParser::parse_assignment() {
...
ExprPtr NewParser::parse_equality() {
...
ExprPtr NewParser::parse_comparison() {
...
ExprPtr NewParser::parse_alternative() {
...
ExprPtr NewParser::parse_additive() {
...
ExprPtr NewParser::parse_multiplicative() {
...
ExprPtr NewParser::parse_postfix() {
...
// Primary expressions
ExprPtr NewParser::parse_primary() {
    // Literals
...
ExprPtr NewParser::parse_index_or_slice(ExprPtr base) {
...
ExprPtr NewParser::parse_field_access(ExprPtr base) {
...
ExprPtr NewParser::parse_function_call(const std::string& name) {
...
ExprPtr NewParser::parse_array_literal() {
    consume(TokenType::LeftBracket, "Expected '['");
...
ExprPtr NewParser::parse_object_literal() {
...
ExprPtr NewParser::parse_parenthesized() {
    consume(TokenType::LeftParen, "Expected '('");
    ExprPtr expr = parse_expression();
    consume(TokenType::RightParen, "Expected ')'");
    return expr;
...
ExprPtr NewParser::parse_reduce() {
...
ExprPtr NewParser::parse_foreach() {
...
ExprPtr NewParser::parse_function_def() {
